...
func test_register_enemy() -> void:
	var enemy: CharacterBody3D = auto_free(CharacterBody3D.new())
...
func test_nearest_player_multiple() -> void:
	var close: CharacterBody3D = auto_free(CharacterBody3D.new())
	add_child(close)
	close.global_position = Vector3(2.0, 0.0, 0.0)
	var far: CharacterBody3D = auto_free(CharacterBody3D.new())
	add_child(far)
...
func test_queries_ignore_freed_players() -> void:
	var valid: CharacterBody3D = auto_free(CharacterBody3D.new())
	add_child(valid)
	valid.global_position = Vector3(5.0, 0.0, 0.0)
	var doomed := CharacterBody3D.new()
	add_child(doomed)
	doomed.global_position = Vector3(1.0, 0.0, 0.0)
