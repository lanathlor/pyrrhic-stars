...
func _point_inside_building(point: Vector3, bld: Dictionary) -> bool:
	...
	return point.x >= bcx - bsx and point.x <= bcx + bsx \
		and point.z >= bcz - bsz and point.z <= bcz + bsz


func _segment_intersects_aabb(a: Vector3, b: Vector3,
...
func _test_navmesh_has_polygons() -> void:
	...
	_check("navmesh_has_polygons", path.size() > 0,
		"Navmesh returned zero points — baking failed")


func _test_path_on_open_street() -> void:
	var path := _get_path(Vector3(5.0, FLOOR_Y, -100.0), Vector3(5.0, FLOOR_Y, -10.0))
	_check("path_on_open_street", path.size() >= 2,
...
func _test_spawn_to_lift_is_near() -> void:
	...
	_check("spawn_to_lift_near", d < 10.0,
		"Path length %0.1fm — expected < 10m" % d)
...
func _test_cross_district_avoids_all() -> void:
	...
	_check("cross_district_avoids_all", violated.is_empty(),
...
func _test_canyon_to_lift() -> void:
	...
	if path.size() < 2:
		...
		return
	var violated := _path_violated_buildings(path)
	_check("canyon_to_lift", violated.is_empty(),
		"Path cuts through buildings: %s" % str(violated))
...
func _test_row_c_uses_street() -> void:
	...
	if path.size() < 2:
		...
		return
	var violated := _path_violated_buildings(path)
	_check("row_c_uses_street", violated.is_empty(),
		"Path cuts through buildings: %s" % str(violated))


func _test_path_from_inside_building() -> void:
	var path := _get_path(Vector3(30.0, FLOOR_Y, -125.0), Vector3(5.0, FLOOR_Y, -45.0))
	_check("path_from_inside_building", path.size() >= 2,
		"No path from inside building A3 to lift")
...
func _test_path_longer_than_straight_line() -> void:
	...
	if path.size() < 2:
		...
		return
	var d := _path_length(path)
	_check("path_longer_than_straight", d > straight,
		...
		for pi in path.size():
			for bi in BUILDINGS.size():
				if _point_inside_building(path[pi], BUILDINGS[bi]):
					found_inside = "Point %d (%s) route %s→%s inside building %d" % [
