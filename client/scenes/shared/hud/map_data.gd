...
static func get_floor_for_position(pos: Vector3) -> Dictionary:
	for floor_def in FLOORS:
		...
		if floor_def.has("bounds_min"):
			...
			if pos.x < bmin.x or pos.x > bmax.x or pos.z < bmin.z or pos.z > bmax.z:
				continue
		return floor_def
	return {}
...
static func _scan_node_recursive(
	node: Node, y_min: float, y_max: float, rects: Array, circles: Array
) -> void:
	if node is CSGBox3D:
		...
		if box_center_y >= y_min and box_center_y <= y_max:
			...
			if area >= 0.1:
				...
				if node_type != "":
					rects.append(
						{
							"rect": Rect2(pos.x - s.x / 2.0, pos.z - s.z / 2.0, s.x, s.z),
							"type": node_type
						}
					)
		...
		if pos.y >= y_min and pos.y <= y_max:
			if r >= 0.3:
				circles.append(
					{"center": Vector2(pos.x, pos.z), "radius": r, "green": _is_green(sph.name)}
				)
...
static func _is_green(node_name: StringName) -> bool:
	...
	return (
		n.contains("hedge")
		or n.contains("flower")
		or n.contains("grass")
		or n.contains("planter")
		or n.contains("tree")
		or n.contains("canopy")
		or n.contains("shrub")
		or n.contains("bush")
	)
