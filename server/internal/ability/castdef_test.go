...
func newEnemyCaster(id uint16, hp float32) *entity.Enemy {
...
func TestCastDef_EnemyMelee_HitsPlayer(t *testing.T) {
	...
	result := eng.CastDef(def, ctx)
	if !result.OK {
		t.Fatalf("CastDef failed: %s", result.Reason)
	}
...
func TestCastDef_EnemyCaster_SkipsGCDCheck(t *testing.T) {
	...
	ctx := &CastContext{Caster: e, Targets: []entity.Target{p}}
	result := eng.CastDef(def, ctx)
	if !result.OK {
		t.Errorf("enemy cast should not be gated by GCD: %s", result.Reason)
	}
}
...
func TestCastDef_EnemyCaster_SkipsCooldownCheck(t *testing.T) {
	...
	// Cast twice — enemies don't track cooldowns through the engine
	ctx := &CastContext{Caster: e}
	r1 := eng.CastDef(def, ctx)
	r2 := eng.CastDef(def, ctx)
	if !r1.OK || !r2.OK {
		t.Error("enemy should be able to cast repeatedly without cooldown gating")
	}
}
...
func TestCastDef_DeadEnemy_Rejected(t *testing.T) {
	...
	result := eng.CastDef(def, ctx)
	if result.OK {
		t.Error("dead enemy should not be able to cast")
	}
...
func TestResolveAoECircle_EnemyCaster(t *testing.T) {
	...
	results := resolveAoECircle(nil, e.CasterPos(), e.CasterID(), []entity.Target{p}, nil, 5.0, 40, combat.SourceEnemyAoE)
...
func TestCastDef_PlayerTargetsPlayer(t *testing.T) {
	...
	ctx := &CastContext{Caster: attacker, Targets: []entity.Target{victim}}
	result := eng.CastDef(def, ctx)
	if !result.OK {
		t.Fatalf("PvP cast failed: %s", result.Reason)
	}
	if len(result.Events) != 1 {
		t.Fatalf("events = %d, want 1", len(result.Events))
	}
